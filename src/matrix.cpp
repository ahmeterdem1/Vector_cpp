...
template <typename T>
Matrix<T>::Matrix(const unsigned int& a, const unsigned int& b, Vector<T>* v_list) {
...
template <typename T>
Vector<T> Matrix<T>::pop(int index) {
...
template <typename T>
void Matrix<T>::insert(int index, const Vector<T>& v) {
    if (v.length != this->b and this->b != 0) throw DimensionError();
    if (this->b == 0) {
        this->b = v.length;
    }
...
template <typename T>
void Matrix<T>::sort(const bool& reverse) {
    if (this->a == 0) return;
    Vector<T>* temp;
...
template <typename T>
template <typename U>
Matrix<typename Matrix<T>::template ctype<U>> Matrix<T>::operator+ (const U& val) const {
...
Matrix<typename Matrix<T>::template ctype<U>> operator+ (const U& val, const Matrix<T>& m) {
    ...
    for (int i = 0; i < m.a; i++) {
        ...
        v = *(m.data->data + i);
        for (int j = 0; j < m.b; j++) {
            temp[j] = static_cast<typename Matrix<T>::template ctype<U>>(*(v->data + j)) + c;
        }
...
template <typename T>
template <typename U>
Matrix<typename Matrix<T>::template ctype<U>> Matrix<T>::operator+ (const Matrix<U>& m) const {
    if ((this->a != m.a) and (this->b != m.b)) throw DimensionError();
    Vector<ctype<U>> new_data[this->a];
    Vector<T>* v;
    Vector<T>* w;
...
template <typename T>
template <typename U>
Vector<typename Matrix<T>::template ctype<U>> Matrix<T>::operator* (const Vector<U>& v) const {
    if (this->b != v.length) throw DimensionError();
    ctype<U> temp[this->a];
    Vector<T>* w;
    for (int i = 0; i < this->a; i++) {
...
template <typename T>
template <typename U>
Matrix<double> Matrix<T>::operator/ (const U& val) const {
    Vector<T>* v;
...
template <typename T>
template <typename U>
Matrix<bool> Matrix<T>::operator|| (const Matrix<U>& m) const {
    if ((this->a != m.a) or (this->b != m.b)) throw DimensionError();
...
template <typename T>
Matrix<bool> Matrix<T>::toBool() const {
    Vector<bool> new_data[this->a];
    Vector<T>* v;
    for (int i = 0; i < this->a; i++) {
        bool temp[this->b];
...
template <typename T>
Matrix<T> Matrix<T>::zero(const unsigned int& a, const unsigned int& b) {
    Vector<T> new_data[a];
    for (int i = 0; i < a; i++) {
        T temp[b];
        for (int j = 0; j < b; j++) {
...
template <typename T>
Matrix<T> Matrix<T>::one(const unsigned int& a, const unsigned int& b) {
    Vector<T> new_data[a];
    for (int i = 0; i < a; i++) {
        T temp[b];
        for (int j = 0; j < b; j++) {
...
template <typename T>
Matrix<T> Matrix<T>::identity(const unsigned int& d) {
    Vector<T> new_data[d];
    for (int i = 0; i < d; i++) {
        T temp[d];
        for (int j = 0; j < d; j++) {
...
template <typename T>
Matrix<int> Matrix<T>::randMint(const unsigned int& m, const unsigned int& n, const int& a, const int& b) {
...
template <typename T>
Matrix<float> Matrix<T>::randMfloat(const unsigned int& m, const unsigned int& n, const float& a, const float& b) {
...
template <typename T>
Matrix<double> Matrix<T>::randMdouble(const unsigned int& m, const unsigned int& n, const double& a, const double& b) {
...
template <typename T>
Matrix<bool> Matrix<T>::randMbool(const unsigned int&m, const unsigned int& n) {
...
template <typename T>
Matrix<double> Matrix<T>::echelon(const double& lowlimit, const double& highlimit) {
    Vector<double>* temp;
    Vector<double>* v;
    Vector<double>* w;
...
template <typename T>
Matrix<double> Matrix<T>::rrechelon(const double& lowlimit, const double& highlimit) {
    Vector<double>* temp;
    Vector<double>* v;
    Vector<double>* w;
    double first, rows_first;
...
template <typename T>
double Matrix<T>::determinant(const std::string& method, const double& lowlimit, const double& highlimit) {
    if (this->a != this->b) throw DimensionError();
    if (this->a == 1) {
...
template <typename T>
Matrix<double> Matrix<T>::inverse(const std::string& method, const unsigned int& resolution,
                                     const double& lowlimit, const double& highlimit) const {
...
template <typename T>
Matrix<double> Matrix<T>::givens(const unsigned int& dim, const unsigned int& i, const unsigned int& j,
                             const double& angle, const unsigned int& resolution) {
...
template <typename T>
template <typename U>
Vector<typename Matrix<T>::template ctype<U>> Matrix<T>::leastSquares(const Vector<U>& v, const std::string& method, const unsigned int& resolution,
                              const double& lowlimit, const double& highlimit) const {
...
template <typename T>
Matrix<double>* Matrix<T>::QR(const double& limiter) const {
    if (this->a == 0) return nullptr;
...
template <typename T>
double* Matrix<T>::eigenvalue(const unsigned int& resolution) const {
...
template <typename T>
template <typename U>
double Matrix<T>::frobeniusProduct(const Matrix<T>& m, const Matrix<U>& n) {
...
template <typename T>
Matrix<T> Matrix<T>::submatrix(int a, int b, int c, int d) const {
